 ...
 */
export const RiseArcEffect: React.FC<EffectComponentProps> = ({
    options,
    onComplete,
    onRegisterStop,
}) => {
    ...
    useEffect(() => {
        ...
        const initCircles = () => {
            ...
            // 3个渐变配置（两个蓝紫色主调，一个橘色主调，透明度约0.05）
            const colorConfigs = [
                // 第一个：蓝紫色主调
                [
                    { offset: 0, color: "rgba(116, 85, 211, 0.06)" },
                    { offset: 0.4, color: "rgba(147, 112, 219, 0.06)" },
                    { offset: 0.7, color: "rgba(138, 43, 226, 0.06)" },
                    { offset: 1, color: "rgba(147, 112, 219, 0.06)" },
                ],
                // 第二个：橘色主调
                [
                    { offset: 0, color: "rgba(255, 140, 0, 0.02)" },
                    { offset: 0.3, color: "rgba(255, 165, 0, 0.02)" },
                    { offset: 0.6, color: "rgba(255, 200, 100, 0.02)" },
                    { offset: 1, color: "rgba(255, 165, 0, 0.02)" },
                ],
                // 第三个：蓝紫色主调
                [
                    { offset: 0, color: "rgba(46, 43, 226, 0.06)" },
                    { offset: 0.4, color: "rgba(112, 117, 219, 0.06)" },
                    { offset: 0.7, color: "rgba(85, 161, 211, 0.04)" },
                    { offset: 1, color: "rgba(112, 117, 219, 0.06)" },
                ],
            ];
        ...
        const animate = (currentTime: number) => {
            ...
            circlesRef.current.forEach((circle) => {
                ...
                const effectiveRiseProgress = Math.min(
                    effectiveElapsed / riseDuration,
                    1
                );
                const easedProgress =
                    1 - Math.pow(1 - effectiveRiseProgress, 3); // 缓动函数

                // 在升起阶段结束后，半径保持最大值
                if (effectiveRiseProgress >= 1) {
                    circle.currentRadius =
                        circle.initialRadius +
                        (targetRadius - circle.initialRadius);
                } else {
                    circle.currentRadius =
                        circle.initialRadius +
                        (targetRadius - circle.initialRadius) * easedProgress;
                }
                ...
                circle.colorStops.forEach((stop) => {
                    const rgbaMatch = stop.color.match(
                        /rgba\((\d+),\s*(\d+),\s*(\d+),\s*([\d.]+)\)/
                    );
                    if (rgbaMatch) {
                        const r = rgbaMatch[1];
